...
class Spotify:
    ...
    def get_tracks_of_playlist(self, spotify_username: str, playlist_name: str) -> pd.DataFrame:
        ...
        playlist_tracks = self.sp.user_playlist_tracks(playlist_owner, playlist_uri, fields='items,uri,name,id,total')["items"]

        tracks_pd_list = list()
        for track in playlist_tracks:
            sub_dict = {k: track["track"][k] for k in ("name", "duration_ms", "artists", "album")}
        ...
        tracks_pd["duration"] = tracks_pd.apply(lambda x: convert_ms_to_readable(x["duration_ms"]), axis=1)
        
        return tracks_pd[["name", "artist", "album","duration", "duration_ms"]]
